...
class BaseWrapper(Monitor):
    ...
    def __init__(self, env, number_of_features: int = 0, training_episode_length: int = 5000000,
                  n_trail="", gamma=0,
                 number_learing_steps=500000, number_past_vals=0, config=None, safe=1, learn_model=0):
        """
        ...
        if safe:
            if learn_model:
                ...
                self.safe_guard = Safeguard(feasible_init, set_scaler=0, state_limits=np.array([300/400, 325 * 1.2/650]),
                                            action_limits=np.array([1]), A_d=A_hat, B_d=B_hat,
                                            ts=self.env.env.time_step_size)
            else:
                self.model_id = None
                self.safe_guard = Safeguard(feasible_set, set_scaler=0, state_limits=np.array([300/400, 325*1.2/650]),
